...
from marker.SpreadsheetMarkerForStudents.usecasesmarker.spread_sheet_factory_for_checker import SpreadSheetFactoryForChecker
import os
    ...
    def check_line(self,line,valor_total,count):
        ...
        if count==1:
            expected=["1","=1+2","=10/(2+3)","=A1*10-5","=(A5*4)/(A2+A2)","=100/(A5+(A5*A5/5))" \
                ,"=(A5*4)/(A2+A2)+SUM(1,2,3,4,5)","=(A5*4)/(A2+A2)+SUM(A1,A2,3,4,5)" \
                ,"=(A5*4)/(A2+A2)+SUM(A1,A2,3,4,5,A6:A12)","=(A5*4)/(A2+A2)+SUM(A1,A2,3,4,5,A6:A12,MIN(A13:A20))"]
